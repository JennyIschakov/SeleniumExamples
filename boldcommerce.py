...
def check_page_url_title(url, title):
    print(divider)
    sleep(1)
    # Checking that we're on the correct URL address and we're seeing correct title
    if driver.current_url == url and driver.title == title:
        print(f'Page URL <{url}> --- loaded successfully')
        print(f'Page title <{title}> is correct')
    else:
...
def contact_sales_form(): # Contact Sales button - on right side
    ...
    # Summarize the form
    print(divider)
    print("Contact Sales form filled successfully, with following details:")
    table = PrettyTable()
    table.field_names = ['Field', 'Value']
    table.add_rows([
        ['Email', email],
        ['First name', filled_first_name],
        ['Last name', filled_last_name],
        ['Phone number', filled_phone],
        ['Company name', filled_company_name],
        ['Website URL', filled_company_web],
        ['Business type', selected_business_type],
        ['eCommerce platform', selected_ecommerce_platform],
        ['Approximate annual revenue', selected_annual_revenue],
    ])
    table.align = 'l'
    print(table)
...
check_page_url_title(home_page_url, 'Bold Commerce | Modular Commerce Solutions - Sell Anywhere')
...
check_page_url_title('https://boldcommerce.com/contact/sales', 'Contact Sales | Bold Commerce')
